...
public class CacheEngineImpl<K, V> implements CacheEngine<K, V> {
    ...
    /** */
    private final TimerForIdle timerForIdle = new TimerForIdle();
    /** */
    ...
    CacheEngineImpl(int maxElements, long lifeTimeMs, long idleTimeMs, boolean isEternal) {
        ...
        if (!isEternal) {
            if(idleTimeMs != 0) {
                timer.schedule(timerForIdle, idleTimeMs, idleTimeMs);//delay = idleTimeMs!!
            }
        }
    }
...
//            }
    ...
    //Эта проверка должна выпоняться каждые idleTimeMs
    private class TimerForIdle extends TimerTask{
        @Override
        public void run(){
            System.out.println("It's time for Idle!!");
            for (Map.Entry<K, MyElement<K, V>> entry : elements.entrySet()) {
                MyElement<K, V> el =  entry.getValue();
                if(el.getValue() == null
                        || el.getLastAccessTime() + idleTimeMs < el.getCurrentTime()){
                    elements.remove(el.getKey());
                }
            }
        }
    }
